...
export class NotificationsGateway implements OnGatewayConnection, OnGatewayDisconnect {
    ...
    async handleConnection(client: AuthenticatedSocket) {
        try {
            ...
            this.logger.log(`Notification client connected: ${client.id} (User: ${userName})`);
    ...
    async handleGetNotifications(
        @ConnectedSocket() client: AuthenticatedSocket,
        @MessageBody() data: { limit?: number; cursor?: string; unreadOnly?: boolean },
    ) {
        try {
            if (!client.userId) {
    ...
    async sendNotification(notification: {
        ...
        this.emitToUser(notification.recipientId, 'unread-count', { count: unreadCount });
    ...
    async broadcastNotification(
        userIds: string[],
        notification: Omit<Parameters<typeof this.sendNotification>[0], 'recipientId'>,
    ) {
