...
import { NotificationDispatcherService, DispatchNotificationPayload } from './services/notification-dispatcher.service';
import { NotificationType, NotificationPriority, GetNotificationsQueryDto } from './dto/notification.dto';
...
export class NotificationsService {
    ...
    async markAsRead(
        userId: string,
        options: { notificationId?: string; notificationIds?: string[]; markAll?: boolean },
    ) {
        const now = new Date();
    ...
    async notifyDeadlineApproaching(
        ...
        const priority = data.hoursRemaining <= 24 ? NotificationPriority.URGENT : NotificationPriority.HIGH;
    ...
    async notifyDocumentExpiring(
        ...
        const priority = data.daysRemaining <= 7 ? NotificationPriority.URGENT : NotificationPriority.HIGH;
    ...
    async notifyBrandValidationComplete(credentialId: string, validationSuccess: boolean) {
        try {
            ...
            return this.notify({
                ...
                priority: validationSuccess ? NotificationPriority.NORMAL : NotificationPriority.HIGH,
                recipientId,
                companyId: credential.brandId,
                title: validationSuccess ? 'Validação de CNPJ Concluída' : 'Validação de CNPJ Falhou',
            ...
            this.logger.error(`Error notifying validation complete: ${error.message}`);
        }
    }
    ...
    async notifyBrandComplianceApproved(credentialId: string) {
        try {
            ...
            this.logger.error(`Error notifying compliance approved: ${error.message}`);
        }
    }
