...
export class NotificationDispatcherService {
  ...
  async dispatch(payload: DispatchNotificationPayload): Promise<{
    ...
    const notification = await this.prisma.notification.create({
      data: {
        ...
        emailStatus: this.shouldSendEmail(payload, preferences)
          ? 'PENDING'
          : 'SKIPPED',
        whatsappStatus: this.shouldSendWhatsapp(payload, preferences)
          ? 'PENDING'
          : 'SKIPPED',
      },
    });
    ...
    if (
      this.shouldSendWhatsapp(payload, preferences) &&
      !payload.skipWhatsapp
    ) {
      this.sendWhatsapp(notification.id, payload, preferences).catch(
        (error) => {
  ...
  private getNotificationCategory(type: NotificationType): string {
    ...
    if (type.startsWith('MESSAGE_') || type.startsWith('PROPOSAL_'))
      return 'messages';
