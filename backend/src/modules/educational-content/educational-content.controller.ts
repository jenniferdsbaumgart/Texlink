...
import { UserRole, EducationalContentType, EducationalContentCategory } from '@prisma/client';
...
export class EducationalContentController {
    ...
    async findAll(
        ...
        return this.educationalContentService.findAll(category, contentType, active);
    }
    ...
    @UseGuards(JwtAuthGuard, RolesGuard)
    @Roles(UserRole.ADMIN)
    async update(@Param('id') id: string, @Body() dto: UpdateEducationalContentDto) {
