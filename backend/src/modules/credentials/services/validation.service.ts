...
export class ValidationService {
    ...
    async processValidation(credentialId: string, performedById: string) {
        ...
        try {
            ...
            if (validationResult.isValid && validationResult.data) {
                ...
                await this.updateCredentialWithValidationData(credentialId, validationResult.data);
                ...
                return {
                    ...
                    message: 'CNPJ validado com sucesso. Próximo passo: análise de compliance.',
                };
                ...
                return {
                    ...
                    message: 'Validação de CNPJ falhou. Verifique o CNPJ e tente novamente.',
                };
            }
            ...
            this.logger.error(`Erro ao processar validação ${credentialId}: ${errorMessage}`);
            ...
            return {
                ...
                error: errorMessage,
                nextStep: 'RETRY',
                message: 'Ocorreu um erro durante a validação. Tente novamente mais tarde.',
            };
        }
    }
    ...
    private async saveValidationResult(credentialId: string, result: any) {
        return this.prisma.credentialValidation.create({
            data: {
                ...
                foundedAt: result.data?.dataAbertura ? new Date(result.data.dataAbertura) : null,
     ...
     */
    private async updateCredentialWithValidationData(credentialId: string, data: any) {
        const updateData: any = {};

        if (data.razaoSocial) updateData.legalName = data.razaoSocial;
        if (data.nomeFantasia && !updateData.tradeName) updateData.tradeName = data.nomeFantasia;
        ...
        if (Object.keys(updateData).length > 0) {
            ...
            this.logger.log(`Dados da empresa atualizados para credential ${credentialId}`);
        }
    }
