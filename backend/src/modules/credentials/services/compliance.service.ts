...
export class ComplianceService {
  ...
  async approveCompliance(credentialId: string, notes: string, user: AuthUser) {
    ...
    if (
      ...
      analysis.manualReviewStatus !== ManualReviewStatus.PENDING
    ) {
      throw new BadRequestException(
    ...
    if (!approvableStatuses.includes(credential.status)) {
      throw new BadRequestException(
    ...
    return {
      ...
      message:
        'Compliance aprovado com sucesso. Credenciamento pode prosseguir para envio de convite.',
  ...
  async rejectCompliance(
    ...
    if (!rejectableStatuses.includes(credential.status)) {
      throw new BadRequestException(
  ...
  private calculateScores(
    ...
    if (creditResult?.score) {
      ...
      if (creditResult.debtAmount && creditResult.debtAmount > 50000) {
        const debtPenalty = Math.min(
          15,
          Math.floor(creditResult.debtAmount / 10000),
        );
    ...
    if (validation?.foundedAt) {
      ...
      const yearsActive =
        (Date.now() - foundedDate.getTime()) / (1000 * 60 * 60 * 24 * 365);
    ...
    switch (riskLevel) {
      case RiskLevel.LOW:
        return {
          action: 'APPROVE',
          reason:
            'Análise de compliance aprovada automaticamente. Baixo risco.',
          requiresManualReview: false,
        };
  ...
  private identifyRiskFactors(
    ...
    if (!flags.hasActiveCNPJ) {
      factors.push(
        'CNPJ não está ativo - Status: ' +
          (validation?.companyStatus || 'Desconhecido'),
      );
    }
    ...
    if (flags.hasNegativeCredit) {
      if (creditResult?.debtAmount) {
        factors.push(
          `Possui negativações no mercado (Total: R$ ${creditResult.debtAmount.toLocaleString('pt-BR', { minimumFractionDigits: 2 })})`,
        );
      } else {
    ...
    if (
      creditResult?.debtAmount &&
      creditResult.debtAmount > 100000 &&
      !flags.hasNegativeCredit
    ) {
    ...
    if (validation?.foundedAt) {
      ...
      if (yearsActive < 1) {
        factors.push(
          'Empresa com menos de 1 ano de atividade (risco de inexperiência)',
        );
      } else if (yearsActive < 2) {
   ...
   */
  private getStatusFromRecommendation(recommendation: {
    action: string;
