...
export class SendGridProvider implements INotificationProvider, OnModuleInit {
    ...
    constructor(private readonly configService: ConfigService) {
        ...
        this.fromEmail = this.configService.get<string>('SENDGRID_FROM_EMAIL', 'noreply@texlink.com.br');
        this.fromName = this.configService.get<string>('SENDGRID_FROM_NAME', 'Texlink');
    }
    ...
    onModuleInit() {
        if (this.apiKey) {
            ...
            this.logger.warn('SendGrid não configurado: SENDGRID_API_KEY não definida');
        }
    }
    ...
    async send(payload: NotificationPayload): Promise<NotificationResult> {
        if (!this.isConfigured) {
            return {
                success: false,
                error: 'SendGrid não configurado. Defina SENDGRID_API_KEY nas variáveis de ambiente.',
                provider: this.name,
                type: this.type,
                timestamp: new Date(),
            };
    ...
    private handleError(error: unknown): { message: string; code?: string; raw?: Record<string, unknown> } {
        ...
        if (this.isResponseError(error)) {
            ...
            const body = typeof responseBody === 'object' ? responseBody as Record<string, unknown> : undefined;
            ...
            if (statusCode === 400) {
                ...
                return {
                    message: (firstError?.message || 'Requisição inválida') as string + fieldError,
            ...
            return {
                message: (firstError?.message || error.message || `Erro HTTP ${statusCode}`) as string,
