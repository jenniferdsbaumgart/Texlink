...
export class ReceitaWsProvider implements ICNPJProvider {
    ...
    async validate(cnpj: string): Promise<CNPJValidationResult> {
        ...
        try {
            this.logger.log(`Validando CNPJ ${this.maskCnpj(cleanCnpj)} via ReceitaWS`);
            ...
            if (rawData.status === 'ERROR') {
                const errorMessage = (rawData.message || 'Erro ao consultar CNPJ') as string;
    ...
    private parseResponse(raw: Record<string, unknown>): CNPJData {
        ...
        // Atividade principal
        const atividadePrincipal = Array.isArray(raw.atividade_principal) && raw.atividade_principal.length > 0
        ...
        const atividadesSecundarias = Array.isArray(raw.atividades_secundarias)
            ? (raw.atividades_secundarias as Record<string, string>[]).map(parseAtividade)
            : [];
        ...
        return {
            ...
            capitalSocial: this.parseCapitalSocial((raw.capital_social || '0') as string),
