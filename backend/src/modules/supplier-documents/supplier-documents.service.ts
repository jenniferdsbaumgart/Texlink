...
import { SupplierDocumentType, SupplierDocumentStatus, CompanyType } from '@prisma/client';
...
export class SupplierDocumentsService {
    ...
    private async getSupplierCompanyId(userId: string): Promise<string> {
        ...
        if (!companyUser) {
            throw new ForbiddenException('You must be associated with a supplier company');
        }
    ...
    async findAll(userId: string, type?: SupplierDocumentType, status?: SupplierDocumentStatus) {
        ...
        const updatedDocs = documents.map(doc => {
            if (doc.expiresAt && doc.fileUrl) {
                ...
                if (newStatus !== doc.status) {
                    ...
                    this.prisma.supplierDocument.update({
                        where: { id: doc.id },
    ...
    async create(dto: CreateSupplierDocumentDto, userId: string) {
        ...
        if (!MONTHLY_DOCUMENTS.includes(dto.type)) {
            ...
            if (existing) {
                throw new BadRequestException('A document of this type already exists. Please update the existing one.');
            }
        }
    ...
    async uploadFile(id: string, file: UploadedFile, userId: string) {
        ...
        if (document.fileUrl) {
            ...
            if (oldKey) {
                await this.storage.delete(oldKey).catch(() => { /* ignore */ });
            }
        }

        // Upload new file
        const { url } = await this.storage.upload(file, `supplier-documents/${companyId}`);
    ...
    async createWithFile(dto: CreateSupplierDocumentDto, file: UploadedFile, userId: string) {
        const companyId = await this.getSupplierCompanyId(userId);
    ...
    async update(id: string, dto: UpdateSupplierDocumentDto, userId: string) {
        ...
        const expiresAt = dto.expiresAt ? new Date(dto.expiresAt) : document.expiresAt;
        const status = expiresAt && document.fileUrl
            ? this.calculateStatus(expiresAt)
            : document.status;
    ...
    async getDocumentChecklist(userId: string) {
        ...
        // Build checklist
        const allTypes = Object.values(SupplierDocumentType) as SupplierDocumentType[];
        return allTypes.map(type => {
            const doc = docMap.get(type);
            if (doc) {
                const status = doc.fileUrl && doc.expiresAt
